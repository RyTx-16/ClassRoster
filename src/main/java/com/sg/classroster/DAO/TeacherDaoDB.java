...
@Repository // Annotation to use JDBCTemplate
public class TeacherDaoDB implements TeacherDao{
    ...
    public Teacher getTeacherById(int id) {
        try {
            ...
            return jdbcTemplateObject.queryForObject(GET_TEACHER_BY_ID, new TeacherMapper(), id);
    ...
    public List<Teacher> getAllTeachers() {
        ...
        return jdbcTemplateObject.query(GET_ALL_TEACHERS, new TeacherMapper());
    }
    ...
    public Teacher addTeacher(Teacher teacher) {
        final String INSERT_TEACHER = "INSERT INTO teacher(firstName, lastName, specialty) " +
                "VALUES(?,?,?)";
        jdbcTemplateObject.update(INSERT_TEACHER,
                teacher.getFirstName(),
                teacher.getLastName(),
                teacher.getSpeciality());

        int newId = jdbcTemplateObject.queryForObject("SELECT LAST_INSERT_ID()", Integer.class);
        teacher.setId(newId);
        return teacher;
    }
    ...
    public void updateTeacher(Teacher teacher) {
        final String UPDATE_TEACHER = "UPDATE teacher SET firstName = ?, lastName = ?, " +
                "specialty = ? WHERE id = ?";
        jdbcTemplateObject.update(UPDATE_TEACHER,
                teacher.getFirstName(),
                teacher.getLastName(),
                teacher.getSpeciality(),
                teacher.getId());    }
    ...
    public void deleteTeacherById(int id) {
        ...
        jdbcTemplateObject.update(DELETE_COURSE_STUDENT, id);

        final String DELETE_COURSE = "DELETE FROM course WHERE teacherId = ?";
        jdbcTemplateObject.update(DELETE_COURSE, id);

        final String DELETE_TEACHER = "DELETE FROM teacher WHERE id = ?";
        jdbcTemplateObject.update(DELETE_TEACHER, id);    }

/*
Implementation of RowMapper interface that maps rows from ResultSet to Teacher Objects
 */
    public static final class TeacherMapper implements RowMapper<Teacher> {
        ...
        public Teacher mapRow(ResultSet rs, int index) throws SQLException {
            ...
            teacher.setSpeciality(rs.getString("specialty"));
